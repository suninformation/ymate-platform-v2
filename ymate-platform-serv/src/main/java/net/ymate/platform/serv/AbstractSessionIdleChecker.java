...
/**
 * @author 刘镇 (suninformation@163.com) on 2018/11/21 9:50 PM
 * @version 1.0
 */
public abstract class AbstractSessionIdleChecker<SESSION_WRAPPER extends ISessionWrapper, SESSION_ID, MESSAGE_TYPE> implements ISessionIdleChecker<SESSION_WRAPPER, SESSION_ID, MESSAGE_TYPE> {

    private static final Log _LOG = LogFactory.getLog(AbstractSessionIdleChecker.class);
     ...
     * 执行会话关闭操作
     *
     * @param sessionWrapper 会话管理器对象
     */
    protected abstract void __doCloseSession(SESSION_WRAPPER sessionWrapper);
    ...
    public void processIdleSession(Map<SESSION_ID, SESSION_WRAPPER> sessions, long idleTimeInMillis) {
        ...
        while (_iterator.hasNext()) {
            ...
            if (System.currentTimeMillis() - _entry.getValue().getLastTouchTime() > idleTimeInMillis) {
                _iterator.remove();
                //
                __doCloseSession(_entry.getValue());
                //
