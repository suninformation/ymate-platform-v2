    ...
    protected DataSourceCfgMeta __doParserDataSourceCfgMeta(String dsName, Map<String, String> _moduleCfgs) throws Exception {
        ...
        if (!_dataSourceCfgs.isEmpty()) {
            ...
            if (StringUtils.isNotBlank(_meta.getConnectionUrl()) && StringUtils.isNotBlank(_meta.getUsername())) {
                ...
                // 数据源适配器
                String _adapterClassName = Type.DS_ADAPTERS.get(StringUtils.defaultIfBlank(_dataSourceCfgs.get("adapter_class"), "default"));
                _meta.setAdapterClass((Class<? extends IDataSourceAdapter>) ClassUtils.loadClass(_adapterClassName, this.getClass()));
                //
                // 连接和数据库类型
                try {
                    _meta.setType(Type.DATABASE.valueOf(StringUtils.defaultIfBlank(_dataSourceCfgs.get("type"), "").toUpperCase()));
                    ...
                    if (_type != null && _type.length > 0) {
                        ...
                        _meta.setType(Type.DATABASE.valueOf(_type[1].toUpperCase()));
                    }
                }
                //
                _meta.setDialectClass(_dataSourceCfgs.get("dialect_class"));
                _meta.setDriverClass(StringUtils.defaultIfBlank(_dataSourceCfgs.get("driver_class"), Type.DB_DRIVERS.get(_meta.getType())));
