 ...
 */
public class PluginBeanHandler implements IBeanHandler {

    private final IPluginFactory pluginFactory;

    public PluginBeanHandler(IPluginFactory pluginFactory) {
        this.pluginFactory = pluginFactory;
    }

    @Override
    public Object handle(Class<?> targetClass) {
        if (ClassUtils.isNormalClass(targetClass) && targetClass.getClassLoader() instanceof PluginClassLoader) {
            Bean bean = targetClass.getAnnotation(Bean.class);
            if (bean != null) {
                pluginFactory.getOwner().getBeanFactory().registerBean(BeanMeta.create(targetClass, bean.singleton()));
            }
        }
        return null;
    }
