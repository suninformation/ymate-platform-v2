...
/**
 * FastJSON JsonWrapper序列化器实现，提供JsonWrapper的序列化和反序列化功能。
 * <p>
 * 设计目的：为FastJSON提供JsonWrapper类型的序列化和反序列化支持。
 * <p>
 * 使用场景：
 * - 当需要使用FastJSON序列化JsonWrapper类型时
 * - 当需要使用FastJSON反序列化JSON到JsonWrapper类型时
 *
 * @author 刘镇 (suninformation@163.com) on 2021/12/25 7:19 PM
 * @since 2.1.0
 */
public class JsonWrapperFastJsonSerializer {

    /**
     * 私有构造函数，防止外部实例化。
     */
    private JsonWrapperFastJsonSerializer() {
    }

    /**
     * JsonWrapper FastJSON序列化器，用于将JsonWrapper类型序列化为JSON。
     */
    public static class Serializer extends AbstractFastJsonSerializer {
    }

    /**
     * JsonWrapper FastJSON反序列化器，用于将JSON反序列化为JsonWrapper类型。
     */
