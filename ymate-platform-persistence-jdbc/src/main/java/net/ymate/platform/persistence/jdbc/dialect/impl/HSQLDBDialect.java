...
public class HSQLDBDialect extends AbstractDialect {
    ...
    /**
     * 构建HSQLDB的UPSERT语句 (MERGE INTO ... USING ... ON ... WHEN MATCHED THEN UPDATE ... WHEN NOT MATCHED THEN INSERT ...)
     *
    ...
    public String buildUpsertSql(Class<? extends IEntity> entityClass, String prefix, IShardingable shardingable, Fields fields) {
        ...
        mergeSql.append("USING (SELECT ${using_clause} FROM (VALUES(0)) AS dummy) AS source ");
    ...
    /**
     * HSQLDB: MERGE INTO ... WHEN NOT MATCHED THEN INSERT ...
     */
    @Override
    @SuppressWarnings("rawtypes")
    public String buildInsertIfNotExistSql(Class<? extends IEntity> entityClass, String prefix, IShardingable shardingable, Fields fields) {
        ...
        return ExpressionUtils.bind("MERGE INTO ${table_name} AS target USING (SELECT ${using_clause} FROM (VALUES(0)) AS dummy) AS source ON ${on_clause} WHEN NOT MATCHED THEN INSERT (${insert_columns}) VALUES (${insert_values})")
                .set("table_name", tableName)
                .set("using_clause", usingClause)
                .set("on_clause", onClause)
                .set("insert_columns", insertParts[0])
