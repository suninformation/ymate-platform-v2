 ...
 * @since 2.1.4
 */
public class OracleDialectTest extends DialectTestBase {
    ...
    public void testGetName() {
        ...
        // Type.DATABASE.ORACLE 的值是 "ORACLE"（大写）
        org.junit.Assert.assertEquals("ORACLE", dialect.getName());
    }

    @Test
    public void testGetSequenceNextValSql() {
        IDialect dialect = createDialect();
        String sequenceSql = dialect.getSequenceNextValSql("user_seq");
        org.junit.Assert.assertEquals("user_seq.nextval", sequenceSql);
    }
    ...
    public void testBuildPagedQuerySql() {
        ...
        // 测试分页查询 - Oracle 使用 ROWNUM
        // page=2, pageSize=10: limit=(2-1)*10=10, 生成 rownum_ > 10 AND rownum_ <= 20
        ...
        assertSqlContains(pagedSql, "rownum");
        assertSqlContains(pagedSql, "rownum_ > 10");
        assertSqlContains(pagedSql, "rownum_ <= 20");

        // 测试 pageSize = 0: limit=(3-1)*0=0, 生成 rownum <= 0
        pagedSql = dialect.buildPagedQuerySql(originSql, 3, 0);
        printSql("testBuildPagedQuerySql (page=3, pageSize=0)", pagedSql);
        assertSqlContains(pagedSql, "rownum <= 0");
    }
    ...
    public void testBuildUpsertSql() {
        ...
        assertSqlContains(sql, "test_user");
        assertSqlContains(sql, "target");
        assertSqlContains(sql, "USING");
        assertSqlContains(sql, "FROM DUAL");
        assertSqlContains(sql, "source");
    ...
    public void testBuildUpsertSqlWithFields() {
        ...
        org.junit.Assert.assertFalse("email should not be in SQL",
                sql.contains("email = source.email"));
    }
