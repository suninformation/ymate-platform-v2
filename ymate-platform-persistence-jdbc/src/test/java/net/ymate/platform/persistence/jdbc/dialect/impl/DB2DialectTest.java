...
@EnableBeanProxy
@EnableDevMode
public class DB2DialectTest extends DialectTestBase {
    ...
    public void testGetName() {
        ...
        // Type.DATABASE.DB2 的值是 "DB2"（大写）
        org.junit.Assert.assertEquals("DB2", dialect.getName());
    }
    ...
    public void testBuildPagedQuerySql() {
        ...
        // 测试分页查询 - DB2 使用 ROW_NUMBER() OVER()
        // page=2, pageSize=10: limit=(2-1)*10=10, offset=10, 生成 BETWEEN 20 AND 10
        ...
        assertSqlContains(pagedSql, "__rn__");
        assertSqlContains(pagedSql, "BETWEEN 20 AND 10");

        // 测试 pageSize = 0: limit=(3-1)*0=0, offset=0, 生成 BETWEEN 0 AND 0
        pagedSql = dialect.buildPagedQuerySql(originSql, 3, 0);
        printSql("testBuildPagedQuerySql (page=3, pageSize=0)", pagedSql);
        assertSqlContains(pagedSql, "BETWEEN 0 AND 0");
    }
    ...
    public void testBuildUpsertSql() {
        ...
        assertSqlContains(sql, "FROM SYSIBM.SYSDUMMY1");
