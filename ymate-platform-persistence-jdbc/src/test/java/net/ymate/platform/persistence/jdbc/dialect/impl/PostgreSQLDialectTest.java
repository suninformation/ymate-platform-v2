...
@EnableBeanProxy
@EnableDevMode
public class PostgreSQLDialectTest extends DialectTestBase {
    ...
    public void testGetName() {
        ...
        // Type.DATABASE.POSTGRESQL 的值是 "POSTGRESQL"（大写）
        org.junit.Assert.assertEquals("POSTGRESQL", dialect.getName());
    }
    ...
    public void testBuildPagedQuerySql() {
        ...
        // 测试分页查询 - PostgreSQL 使用 LIMIT count OFFSET offset 格式
        // page=2, pageSize=10: limit=(2-1)*10=10, 生成 LIMIT 10 OFFSET 10
        String pagedSql = dialect.buildPagedQuerySql(originSql, 2, 10);
        printSql("testBuildPagedQuerySql (page=2, pageSize=10)", pagedSql);
        assertSqlEquals(pagedSql, "SELECT * FROM test_user LIMIT 10 OFFSET 10");

        // 测试第一页: page=1, pageSize=20: limit=0, 生成 LIMIT 20 OFFSET 0
        pagedSql = dialect.buildPagedQuerySql(originSql, 1, 20);
        printSql("testBuildPagedQuerySql (page=1, pageSize=20)", pagedSql);
        assertSqlEquals(pagedSql, "SELECT * FROM test_user LIMIT 20 OFFSET 0");

        // 测试 pageSize = 0: limit=(3-1)*0=0, 生成 LIMIT 0
        pagedSql = dialect.buildPagedQuerySql(originSql, 3, 0);
        printSql("testBuildPagedQuerySql (page=3, pageSize=0)", pagedSql);
        assertSqlEquals(pagedSql, "SELECT * FROM test_user LIMIT 0");
    }
    ...
    public void testBuildUpsertSql() {
        ...
        assertSqlContains(sql, "INSERT INTO");
        assertSqlContains(sql, "\"test_user\"");
        assertSqlContains(sql, "ON CONFLICT");
        assertSqlContains(sql, "\"id\"");
        assertSqlContains(sql, "DO UPDATE SET");

        // 验证更新部分包含非主键字段，使用 EXCLUDED 语法
        assertSqlContains(sql, "\"name\" = EXCLUDED.\"name\"");
        assertSqlContains(sql, "\"age\" = EXCLUDED.\"age\"");
        assertSqlContains(sql, "\"email\" = EXCLUDED.\"email\"");
    ...
    public void testBuildUpsertSqlCompositeKey() {
        ...
        // 验证包含所有主键在 CONFLICT 子句中（注意：字段名会被转换为下划线命名）
        assertSqlContains(sql, "\"order_id\"");
        assertSqlContains(sql, "\"product_id\"");

        // 验证更新部分包含非主键字段
        assertSqlContains(sql, "\"quantity\" = EXCLUDED.\"quantity\"");
        assertSqlContains(sql, "\"price\" = EXCLUDED.\"price\"");
    }
    ...
    public void testBuildInsertIfNotExistSql() {
        ...
        // 验证使用 INSERT ... ON CONFLICT ... DO NOTHING
        assertSqlContains(sql, "INSERT INTO");
        assertSqlContains(sql, "\"test_user\"");
        assertSqlContains(sql, "ON CONFLICT");
        assertSqlContains(sql, "DO NOTHING");
