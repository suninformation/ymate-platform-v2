 ...
 * @since 2.1.3
 */
@Converter(from = DateTimeValue.class, to = Date.class)
public class DateTimeValueToDateConverter implements IConverter<Date> {

    @Override
    public Date convert(Object target) {
        if (target instanceof DateTimeValue) {
            return ((DateTimeValue) target).getStartDate();
        }
        return null;
    }
}
