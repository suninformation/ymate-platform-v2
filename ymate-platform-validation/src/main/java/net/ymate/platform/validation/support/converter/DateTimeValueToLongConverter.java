 ...
 * @since 2.1.3
 */
@Converter(from = DateTimeValue.class, to = Long.class)
public class DateTimeValueToLongConverter implements IConverter<Long> {

    @Override
    public Long convert(Object target) {
        if (target instanceof DateTimeValue) {
            return ((DateTimeValue) target).getStartDateTimeMillisOrNull();
        }
        return null;
    }
}
